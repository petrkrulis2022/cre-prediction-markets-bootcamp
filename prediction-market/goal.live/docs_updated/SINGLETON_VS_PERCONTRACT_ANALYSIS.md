...
### **5. Log Trigger — Settlement Verification**
...
steps:
  - process-event:
      script: |
        const { matchId, scorerId, totalPayout } = eventData;

        // HTTP POST to your backend with secret API key
        const response = await Functions.makeHttpRequest({
          ...
          headers: {
            ...
            "Content-Type": "application/json",
            "X-Signature": crypto.sha256(`${matchId}${scorerId}${secrets.WEBHOOK_SECRET}`)
          },
          data: {
            matchId,
            scorerId,
...
secrets: BACKEND_API_KEY: "encrypted:abcd1234..."; // Chainlink encrypts this
GOALSERVE_KEY: "encrypted:efgh5678...";
WEBHOOK_SECRET: "encrypted:ijkl9012..."; // For HMAC signing

// In script:
const apiKey = secrets.BACKEND_API_KEY; // Decrypted at runtime
```
...
| Stage                              | Latency | Total    |
| ---------------------------------- | ------- | -------- |
| Contract emits `MatchSettled`      | <1s     | <1s      |
| Log appears in RPC node mempool    | 1-2s    | 1-3s     |
| CRE Log Trigger fires              | <2s     | <9s      |
| Chainlink Functions execute script | 2-5s    | <15s     |
| Backend receives and processes     | 1-2s    | **<17s** |

**Sub-30 seconds is realistic.** ✅

**Answer to Q5:**

> - Use **CRE Log Trigger** (simpler, faster, cheaper than Automation)
> - ✅ Can make outbound HTTP POST with secret API key via encrypted secrets
> - ✅ Latency: sub-30 seconds realistic, often <20 seconds
> - Recommended: HMAC-sign the request body with a webhook secret for integrity

---

### **6. Architecture Recommendation**

**FINAL RECOMMENDATION: Singleton + Persistent CRE Job**
...
CRE HTTP Job (Persistent)
├── Trigger: Every 30 seconds during live hours
├── Query Supabase: "SELECT matchId WHERE isActive=true"
├── For each matchId:
│  ├── Poll Goalserve API
│  ├── If goal detected → call settleMatch(matchId, scorerId)
│  └── If FT no goals → call settleMatch(matchId, 0)
└── Deactivate in contract via isActive flag

CRE Log Trigger (Persistent)
├── Listens to: MatchSettled event on singleton contract
├── On event fired:
│  ├── Extract matchId, scorerId, payout
│  ├── HTTP POST to backend: /api/settle
│  └── Update Supabase: mark bets as RESOLVED, process payouts
└── Alert if backend returns error

Supabase (Single source of truth)
├── matches table: matchId, isActive, settled_at
├── bets table: matchId, playerId, amount, status
└── payouts table: matchId, playerId, amount, claimed_at
```

**Why this wins at your scale:**
...
- ✅ Write CRE HTTP job YAML
- ✅ Register job with Chainlink CRE
- ✅ Set up encrypted secrets (GOALSERVE_KEY, BACKEND_API_KEY)
...
| Question                    | Singleton                          | Per-Match               | Recommendation      |
| --------------------------- | ---------------------------------- | ----------------------- | ------------------- |
| 1. Job targeting            | One job, dynamic matchId iteration | One job per contract    | **Singleton**       |
| 2. Log Trigger registration | One listener forever               | Register per deployment | **Singleton**       |
| 3. `matchId` format         | `keccak256(uuid)` bytes32          | Same issue              | `keccak256()`       |
| 4. HTTP polling freq        | 30s viable                         | Same                    | **30s**             |
| 5. Settlement verification  | CRE Log Trigger + HTTP POST        | Same                    | **CRE Log Trigger** |
| 6. Overall architecture     | **✅ RECOMMENDED**                 | ❌ Not recommended      | **Singleton**       |
